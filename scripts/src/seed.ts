  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      const transaction = {
        ...
        customer_id: `cust_${String(faker.datatype.number({ min: 1, max: 100 })).padStart(3, '0')}`,
        card_id: `card_${String(faker.datatype.number({ min: 1, max: 200 })).padStart(3, '0')}`,
        mcc,
        merchant,
        amount,
        currency: 'INR',
        ts: faker.date.between({ from: '2024-01-01', to: '2025-06-30' }),
        device_id: `dev_${faker.datatype.number({ min: 1, max: 50 })}`,
        geo: {
...
async function createDevices(customerCount: number) {
  ...
  const devices = [];
  for (let i = 1; i <= customerCount; i++) {
    const deviceCount = faker.datatype.number({ min: 1, max: 3 });
    for (let j = 1; j <= deviceCount; j++) {
      const device = {
        id: `dev_${String((i - 1) * 3 + j).padStart(2, '0')}`,
        customer_id: `cust_${String(i).padStart(3, '0')}`,
...
async function createKnowledgeBase() {
  ...
  const kbDocs = [
    ...
    },
    {
      ...
      chunks: ['travel_notice', 'international_transactions', 'fraud_prevention'],
    },
  ];
